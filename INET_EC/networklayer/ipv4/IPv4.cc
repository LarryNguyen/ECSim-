...
void IPv4::handleIncomingDatagram(IPv4Datagram *datagram, const InterfaceEntry *fromIE)
{
    ...
    /***** LARRY modify for cache service ********/

    if(st){
        EV << "LARRY ~~~ [IPv4]st is NOT NULL \n";
        EV << "LARRY ~~~ [IPv4]handleIncomingDatagram : " << datagram->getName() << datagram->getDestAddress() <<"\n";

        //Print list of service of Service Table
        st->printServiceList();

        if(st->getNumberOfService() > 0){
            IPv4Address oldDest = datagram->getDestAddress();
            if(st->isServiceCached(oldDest)){
                EV << "[IPv4] This service is cached" << "\n";
                cModule *host = getContainingNode(this);
                cStringTokenizer tokenizer(host->getFullName());
                const char *token;

                token = tokenizer.nextToken();
                L3Address newDest;
                L3AddressResolver().tryResolve(token, newDest);
                if (newDest.isUnspecified())
                    EV_ERROR << "cannot resolve destination address: " << token << endl;
                else{
                    datagram->setDestinationAddress(newDest);

                    // Keep the time at last request service
                    int index = st->getServiceIndex(oldDest);
                    if(index >= 0){
                        st->setLastRequestSecond(index, simTime());
                    }
                }
            }else{
                EV << "[IPv4] This service is NOT cached yet" << "\n";
            }
        }else{
            EV << "[IPv4] Service Table is EMPTY" << "\n";
        }

            EV << "LARRY ~~~ [IPv4] handleIncomingDatagram : " << datagram->getName() << datagram->getDestAddress() <<"\n";
    }
    else{
        EV << "LARRY ~~~ [IPv4]Service Table is NULL \n";
    }


    /***** END ~~~ LARRY modify for cache service ********/
...
void IPv4::datagramLocalOut(IPv4Datagram *datagram, const InterfaceEntry *destIE, IPv4Address requestedNextHopAddress)
{
    ...
    if (datagram->getDestAddress().isMulticast()) {
        ...
        if (rt->isLocalAddress(destAddr)) {
            EV_INFO << "LARRY ~~~ 2222 Delivering " << datagram << " locally.\n";
...
{
    EV_INFO << "LARRY ~~~ 111 Delivering " << datagram << " locally.\n";
